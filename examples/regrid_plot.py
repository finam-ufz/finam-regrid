...
source = fm.components.StaticSimplexNoise(
    info=fm.Info(grid=in_grid, units=""),
    frequency=0.05,
    octaves=3,
    persistence=0.5,
)
...
specs = GridSpecPlot(["Unif", "Unif2", "Orig", "Points"], title="Grid specifications")

regrid_unif = Regrid(out_grid=uniform_grid, extrap_method=ExtrapMethod.NEAREST_IDAVG)
regrid_unif_2 = Regrid(regrid_method=RegridMethod.CONSERVE_2ND, out_grid=uniform_grid_2)
